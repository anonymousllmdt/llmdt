...
# from action2motion
def calculate_diversity_multimodality(activations, labels, num_labels=4, seed=None, unconstrained = False):
    diversity_times = 200
...
def convert_x_to_rot6d(x):
    # ACTOR use pytorch3d for conversion
    return matrix_to_rotation_6d(axis_angle_to_matrix(x))


def initialize_model(device, modelpath='support_py/actionrecognition/uestc_rot6d_stgcn.tar'):
    # use UESTC-ROT6D action recognition model trained by the authors of ACTOR
...
@torch.no_grad()
def compute_features_labels(model, joints):
    """
    Args:
        joints: (K, J, C, T)
    Return:
    ...
    batch = {'x': joints}
    model(batch)
    features = batch['features'].reshape(K, -1)
    batch_prob = batch["yhat"]
    batch_pred = batch_prob.max(dim=1).indices.reshape(1, K)
    return features, batch_pred


@torch.no_grad()
def compute_features(model, joints):
    """
    Args:
        joints: (K, J, C, T)
    Return:
...
# ---------- STGCN (copied from ACTOR) ----------
# https://github.com/Mathux/ACTOR/blob/master/src/recognition/models/stgcn.py
# modified Graph: __init__ : kintree_path
...
class Graph:
    ...
    Args:
        ...
        - spatial: Spatial constructuration
    ...
    def __init__(self,
                 layout='openpose',
                 strategy='uniform',
                 kintree_path='lib/utils/evaluate/actionrecognition/kintree_table.pkl',
                 max_hop=1,
                 dilation=1):
...
class STGCN(nn.Module):
    r"""Spatial temporal graph convolutional dualt.
    Args:
    ...
    def __init__(self, in_channels, num_class, graph_args,
        ...
        self.st_gcn_dualt = nn.ModuleList((
        ...
        if edge_importance_weighting:
            self.edge_importance = nn.ParameterList([
                ...
                for i in self.st_gcn_dualt
            ])
        else:
            self.edge_importance = [1] * len(self.st_gcn_dualt)
    ...
    def forward(self, batch):
        ...
        # forward
        for gcn, importance in zip(self.st_gcn_dualt, self.edge_importance):

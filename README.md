...
We propose LLMDT, an LLM-based dual-transformer framework for text-driven 3D human motion generation in dynamic scenes. By combining LLM-based spatial reasoning, object-centric scene understanding, trajectory planning, and a pose-aware dual-transformer with hierarchical RVQ, the framework generates realistic, collision-free, and context-aware human motions that outperform existing state-of-the-art methods on the HUMANISE and HumanML3D datasets.
...
## Training on the HumanML3D Dataset

The models are trained on the HumanML3D dataset to learn discrete motion representations and text-conditioned human motion generation.

### Train the RVQ Model

We first train the residual vector quantization (RVQ) model to learn a hierarchical discrete representation of human motion:

```bash
python dualt/vq_trainer.py \
    --name rvq_name \
    --gpu_id 1 \
    --dataset_name t2m \
    --batch_size 256 \
...
Using the pretrained RVQ model, we train the masked transformer for text-conditioned motion token prediction:

```bash
python dualt/train_t2m_mask.py \
    --name mtrans_name \
    --gpu_id 2 \
    --dataset_name t2m \
    --batch_size 64 \
...
Finally, we train the residual transformer to refine the motion representations generated by the masked transformer:

```bash
python dualt/train_t2m_res.py \
    --name rtrans_name \
    --gpu_id 2 \
    --dataset_name t2m \
    --batch_size 64 \

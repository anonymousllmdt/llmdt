...
from utils.smplx_utils import make_smplx, load_smpl_faces
from utils.normalize import make_normalizer
...
class loss(nn.Module):
    def __init__(self, net, cfg,
        ...
        self.pre_methods = loss_cfg.pre_methods
        self.post_methods = loss_cfg.post_methods
        self.sup_methods = loss_cfg.sup_methods
        self.vis_methods = loss_cfg.vis_methods
        ...
        # smplx model
        self.smplx_model = make_smplx(loss_cfg.smplx_model_type)
        self.smplx_faces = torch.from_numpy(load_smpl_faces())
        
        self._build_text_model(loss_cfg)
...
@loss.register()
class Motiondualtloss(loss):
    def __init__(self, net, cfg, loss_cfg) -> None:
        super().__init__(net, cfg, loss_cfg)
        self.coord = cfg.net_cfg.coord
        self.normalizer = None
        if loss_cfg.get('normalizer', None) is not None:
            self.normalizer = self._build_normalizer(loss_cfg.normalizer)
...
class TwoStageloss(nn.Module):
    def __init__(self, path_net, motion_net, cfg,
        ...
        self.pre_methods = traj_cfg.loss_cfg.pre_methods
        self.post_methods = motion_cfg.loss_cfg.post_methods

...
@VIS.register()
def vis_sample_two_stage(loss, batch, vis3d: Wis3D):
    ...
    coord = loss.coord
    # gt_verts = process_smplx_ksamples(loss, batch[f'smplx_params_{coord}'], B*SM, T)
    pd_verts = process_smplx_ksamples(loss, batch[f'smplx_params_{coord}'], B*SM, T, transl=transl, orient=orient, body_pose=body_pose)
    pd_verts = pd_verts.reshape(B, SM, T, -1, 3)
    motion_mask = batch['motion_mask'][0]
    last_frame = (~motion_mask).sum() - 1
    scene_points, target = get_scene_target(batch, loss.coord)
...
def process_smplx_ksamples(loss, smplx_params, K, S, transl=None, orient=None, body_pose=None):
    '''
    Input:
        K, S: shape

...
import torch
from loss.preprocess import axis_angle_to_rot_6d
SUP = Registry('supervision')


@SUP.register()
def get_gt_smplx_params(loss, batch):
...
@SUP.register()
def get_object2pelvis(loss, batch):
    target_pelvis_xy = []
    ...
    # from utils.vis3d_utils import make_vis3d
...
@SUP.register()
def get_gt_zero_vertices(loss, batch):
...
@SUP.register()
def get_gt_vertices(loss, batch):
    B, S, _ = batch['smplx_params']['transl'].shape
    smplx_params = {
        'betas': batch['smplx_params']['betas'],

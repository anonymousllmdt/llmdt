from utils import offscreen_flag
import os
import argparse
import pickle
...
from construct import make_cfg
from datasets.make_dataset import make_data_loader
from dualt.make_network import make_network
from loss.loss import TwoStageloss
from loss.two_stage import save_sample
from utils.net_utils import to_cuda, load_network
from utils.comm import seed_everything
...
def main(cfg):
    ...
    # whole network
    network = TwoStageloss(traj_network, motion_network, cfg, traj_cfg, motion_cfg).eval().cuda()
    run_network(network)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cfg_file", "-c", default='constructs/test/generate.yaml')

...
# Filename extensions for loading constructs from files
_YAML_EXTS = {"", ".yaml", ".yml"}
_PY_EXTS = {".py"}
...
class CfgNode(dict):
    """
    CfgNode represents an internal node in the constructuration tree. It's a simple
    ...
    def __init__(self, init_dict=None, key_list=None):
        ...
        # yaml constructs, you can add the full construct key as a string to the set below.
        self.__dict__[CfgNode.DEPRECATED_KEYS] = set()
        # Renamed options
        # If you rename a construct option, record the mapping from the old name to the new
        # name in the dictionary below. Optionally, if the type also changed, you can
        # make the value a tuple that specifies first the renamed key and then
        # instructions for how to edit the construct file.
    ...
    def merge_from_file(self, cfg_filename):
        """Load a yaml construct file and merge it this CfgNode."""
    ...
    def merge_from_list(self, cfg_list):
        """Merge construct (keys, values) in a list (e.g., from command line) into
    ...
    def key_is_deprecated(self, full_key):
        ...
        if full_key in self.__dict__[CfgNode.DEPRECATED_KEYS]:
            logger.warning("Deprecated construct key (ignoring): {}".format(full_key))
            return True
        return False
    ...
    def raise_key_rename_error(self, full_key):
        ...
        if isinstance(new_key, tuple):
            ...
            msg = ""
        raise KeyError(
            "Key {} was renamed to {}; please update your construct.{}".format(
...
def _load_cfg_from_file(file_obj):
    """Load a construct from a YAML file or a Python source file."""
...
def _load_cfg_from_yaml_str(str_obj):
    """Load a construct from a YAML string encoding."""
    # cfg_as_dict = yaml.safe_load(str_obj)
    cfg_as_dict = yaml.load(str_obj, yaml.FullLoader)
    return CfgNode(cfg_as_dict)


def _load_cfg_py_source(filename):
    """Load a construct from a Python source file."""
    module = _load_module_from_file("yacs.construct.override", filename)
    _assert_with_logging(
        hasattr(module, "cfg"),
...
def _merge_a_into_b(a, b, root, key_list):
    """Merge construct dictionary a into construct dictionary b, clobbering the
...
def _decode_cfg_value(v):
    """Decodes a raw construct value (e.g., from a yaml construct files or command
    line argument) into a Python object.
    """
    # constructs parsed from raw yaml will contain dictionary keys that need to be
...
def _check_and_coerce_cfg_value_type(replacement, original, key, full_key):
    ...
    raise ValueError(
        "Type mismatch ({} vs. {}) with values ({} vs. {}) for construct "
        "key: {}".format(
...
def _load_module_from_file(name, filename):
    if _PY2:
        ...
        spec = importutil.spec_from_file_location(name, filename)

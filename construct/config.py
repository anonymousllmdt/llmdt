...
from utils import logger, logger_activate_debug_mode
...
def make_cfg(args):
    ...
    logger.info(f'Making construct: {args.cfg_file}')  
    # 0. merge all constructs
    cfg = _cfg.clone()
    default_cfg_path = f"constructs/{args.cfg_file.split('/')[1]}/default.yaml"
...
def save_cfg(cfg: CfgNode, resmue=False):
    filename = f'{cfg.record_dir}/construct'
    if resmue:
        filename += '.resume'
    filename += '.yaml'
